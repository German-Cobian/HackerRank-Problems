// Sam's house has an apple tree and an orange tree that yield an abundance of fruit. Using the information 
// given below, determine the number of apples and oranges that land on Sam's house.

// In the diagram below:

// The red region denotes the house, where s is the start point, and t is the endpoint. The apple tree is to the 
...
// Given the value of d for m apples and n oranges, determine how many apples and oranges will fall on Sam's 
...
// and  oranges = [3,-2,-4] units distance. Adding each apple distance to the position of the tree, they land 
// at [4+2,4+3,4+-4] = [6,7,0]. Oranges land at [12+3,12+-2,12+-4] = [15,10,8]. One apple and two oranges land
...
function countApplesAndOranges(s, t, a, b, apples, oranges) {
  ...
  for (let i = 0; i < apples.length; i++) {
    let applesLoc = apples[i] + a;

      if ((applesLoc >= s) && (applesLoc <= t)) {
        applesScore++;
    }
  }
  
  for (let i = 0; i < oranges.length; i++) {
      let orangesLoc = oranges[i] + b;
      
        if ((orangesLoc >= s) && (orangesLoc <= t)) {
          orangesScore++;
      }
  }
  console.log(applesScore);
  console.log(orangesScore);
}