...
// When they flip page 1, they see pages 2 and 3. Each page except the last page will always be printed on both sides. The 
// last page may only be printed on the front, given the length of the book. If the book is n pages long, and a student wants
// to turn to page p, what is the minimum number of pages to turn? They can start at the beginning or the end of the book.
...
function pageCount(n, p) {
  ...
  fromFront = Math.floor(p/2);
  fromBack = Math.floor(n/2) - Math.floor(p/2)
...
let countfwd = Math.floor(p/2);
let countbck = Math.floor(n/2) - Math.floor(p/2);

  if(countbck > countfwd) {
      return countfwd;
  } else {
      return countbck;
  }
